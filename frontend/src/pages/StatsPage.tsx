...
function withIntensity(color: string, intensity: number): string {
...
function HourlyHeatmap({ archives, dateFrom, dateTo }: { archives: ArchiveSlim[]; dateFrom: string; dateTo: string }) {
  const { days, hourlyCounts, maxCount } = useMemo(() => {
    ...
    const counts: Record<string, { total: number; private: number }> = {};
    let max = 0;
    archives.forEach((archive) => {
      const date = parseUTCDate(archive.created_at);
      if (!date) return;
      const dayKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
      const k = `${dayKey}-${date.getHours()}`;
      const entry = counts[k] || { total: 0, private: 0 };
      entry.total += 1;
      if (isPrivateJob(archive)) entry.private += 1;
      counts[k] = entry;
    ...
    <div className="w-full overflow-x-auto">
        ...
        {days.map(day => (
          <div key={day.key} className="flex items-center" style={{ gap }}>
            ...
            {Array.from({ length: 24 }, (_, hour) => {
              const countData = hourlyCounts[`${day.key}-${hour}`] || { total: 0, private: 0 };
              const count = countData.total;
              const privateCount = countData.private;
              const psiCount = count - privateCount;
              const intensity = count / maxCount;
              const privatePct = count > 0 ? (privateCount / count) * 100 : 0;
              const bg =
                count === 0
                  ? '#1f1f1f'
                  : privateCount === 0
                    ? withIntensity(PSI_COLOR, intensity)
                    : psiCount === 0
                      ? withIntensity(PRIVATE_COLOR, intensity)
                      : `linear-gradient(90deg, ${withIntensity(PRIVATE_COLOR, intensity)} ${privatePct}%, ${withIntensity(PSI_COLOR, intensity)} ${privatePct}%)`;
              return (
                <div
                  key={hour}
                  className="rounded-sm"
                  style={{ width: cellSize, height: cellSize, background: bg }}
                  title={`${day.label} ${HOUR_LABELS[hour]}: ${count} print${count !== 1 ? 's' : ''} (PSI ${psiCount}, Private ${privateCount})`}
                />
              );
            })}
          </div>
      ...
      <div className="flex items-center gap-2 mt-3 text-bambu-gray text-xs">
        ...
        <div className="flex" style={{ gap }}>
          ...
          <div className="rounded-sm" style={{ width: cellSize, height: cellSize, background: withIntensity(PSI_COLOR, 0.25) }} />
          <div className="rounded-sm" style={{ width: cellSize, height: cellSize, background: withIntensity(PRIVATE_COLOR, 0.75) }} />
          <div
            className="rounded-sm"
            style={{
              width: cellSize,
              height: cellSize,
              background: `linear-gradient(90deg, ${withIntensity(PRIVATE_COLOR, 1)} 50%, ${withIntensity(PSI_COLOR, 1)} 50%)`,
            }}
          />
        </div>
        <span>More</span>
...
function PrintActivityWidget({
  ...
  const privateDates = archives.filter(isPrivateJob).map((a) => a.created_at);
  return <PrintCalendar printDates={printDates} privatePrintDates={privateDates} months={months} />;
}
