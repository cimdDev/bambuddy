...
import { AlertTriangle, Calendar, ChevronRight, Clock, Coins, MessageSquare } from 'lucide-react';
...
export function PrinterQueueWidget({ printerId, printerModel, loadedFilamentTypes, loadedFilaments, loadedVariants, variant = 'card' }: PrinterQueueWidgetProps) {
  ...
  const renderSlicerUserBadge = (item?: typeof nextItem) => {
    ...
    if (slicerUser) {
      ...
      return (
        <span role="button" tabIndex={0} onClick={openEditor} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') openEditor(e); }} className="rounded-full" title={t('queue.editSlicerUser.editExisting')}>
    ...
    return <span role="button" tabIndex={0} onClick={openEditor} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') openEditor(e); }} title={t('queue.editSlicerUser.addMissing')}>{warning}</span>;
  };
  ...
  return (
    <>
      <Link to="/queue" className={linkClassName}>
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm text-white truncate">{nextItem ? queueItemDisplayName(nextItem) : ''}</p>
              <div className="mt-1 flex flex-wrap items-center gap-2">
                {renderSlicerUserBadge(nextItem)}
                {nextItem?.private_job && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-fuchsia-500/10 text-fuchsia-300 border border-fuchsia-500/20">{t('queue.accounting.privateJob')}</span>}
                {nextCost != null && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-bambu-dark-tertiary text-bambu-gray-light"><Coins className="w-3 h-3" />{formatCurrencyAmount(nextCost, currencySymbol)}</span>}
              </div>
              {nextItem?.comment && (
                <p className="mt-1 flex items-center gap-1 truncate text-xs text-bambu-gray-light">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {totalPending > 1 && <span className="text-xs px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-400/20 text-yellow-700 dark:text-yellow-400 rounded">+{totalPending - 1}</span>}
