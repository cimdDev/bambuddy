...
PSI_COLUMNS: dict[str, list[tuple[str, str]]] = {
    ...
    "spool": [
        ("psi_cost_auto", "BOOLEAN"),  # cost_per_kg was filled from the material price
    ],
}
...
def _psi_columns(table: str) -> list[Column]:
    ...
    types |= {"VARCHAR(20)": String(20), "TEXT": Text(), "BOOLEAN": Boolean()}
...
archives = Table(
    ...
    Column("print_time_seconds", Integer),
...
spools = Table(
    ...
    Column("cost_per_kg", Float),  # upstream column; PSI fills it from the material price
    Column("archived_at", DateTime),
    *_psi_columns("spool"),
)

#: Shadow table per upstream table name, for the migration and the drift test.
SHADOW_TABLES = {t.name: t for t in (archives, library, queue, runs, spools)}

#: PSI's own table: the price per kg of each material, applied to spools
#: without a price of their own (see :mod:`.material_prices`).
material_prices = Table(
    "psi_material_prices",
    psi_metadata,
    Column("material", String(50), primary_key=True),  # upper-case, trimmed
    Column("cost_per_kg", Float, nullable=False),
)
