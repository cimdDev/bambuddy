...
* :mod:`.material_prices` default price per material, written onto spools
* :mod:`.migrations`      PSI columns and carry-over of earlier builds' data
* :mod:`.permissions`     ``psi_accounting:read``, added to upstream's registry
...
def install(app: FastAPI) -> None:
    ...
    from backend.app.custom.psi import (  # noqa: F401 — registers migrations and the ORM hooks
        material_prices,
        migrations,
        permissions,
        runs,
    ...
    permissions.register()  # before init_db, whose admin sync then grants it
