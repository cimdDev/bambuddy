...
async def chart_runs(db: AsyncSession, conditions: list, *, limit: int, offset: int = 0) -> list[dict]:
    """Upstream's ``GET /archives/slim`` rows, each with its job type.

    Same rows, order, paging and fields the stats charts read, so the PSI
    copies of those charts draw the same totals, split into PSI and private.
    Material does not matter here: every private class is ``private``.
    """
    stmt = (
        select(
            runs.c.printer_id,
            runs.c.print_name,
            archives.c.print_time_seconds,
            runs.c.started_at,
            runs.c.completed_at,
            runs.c.duration_seconds,
            runs.c.filament_used_grams,
        ...
        .select_from(runs.outerjoin(archives, archives.c.id == runs.c.archive_id))
        .where(*conditions)
    ...
    return [
        {
            ...
            "actual_time_seconds": _measured_seconds(r.duration_seconds, r.started_at, r.completed_at),
            "filament_used_grams": r.filament_used_grams,
            "status": r.status,
            "started_at": r.started_at,
            "created_at": r.created_at,
            "job": classification.job_type(r.psi_class),
        }
        for r in (await db.execute(stmt)).all()
    ]


def _measured_seconds(duration_seconds: int | None, started_at: datetime | None, completed_at: datetime | None):
    """``actual_time_seconds`` as upstream's slim listing computes it: ``None``, not 0, when unknown."""
    if duration_seconds is not None:
        return duration_seconds
    if started_at and completed_at and (completed_at - started_at).total_seconds() > 0:
