...
* the accounting endpoints need ``stats:read``, and filtering by Bambuddy user
  needs ``stats:filter_by_user`` as it does on upstream's stats page.
"""
...
from backend.app.custom.psi import accounting, classification, service
...
async def get_accounting(
    ...
    await db.commit()
    return accounting.summarize(rows)
