...
1. **Overlay, not patch.** PSI code lives in PSI directories. Upstream files
   are touched only at *seams*: one import and one insertion, marked
   `PSI-SEAM`. There are nine seam files in total (§4.1), all insert-only.
...
| `App.tsx` | import from `./custom/psi/app` + `<Route path="psi" …>` | next to the other routes inside the `Layout` route. Importing `custom/psi/app` also inserts the sidebar entry into upstream's exported `defaultNavItems` (after `stats`), so `Layout.tsx` itself is not touched — upstream's tests pin that list. |

Each seam file also has one `import … from '…/custom/psi'; // PSI-SEAM`.

### 4.2 Resolving a rebase conflict

Conflicts can only happen in the nine seam files, because nothing else of
...
| stopped exporting `defaultNavItems` from `Layout.tsx`, or froze it | `tsc` fails in `custom/psi/nav.ts`, or the sidebar entry is gone | find the new nav registry; worst case add a `Layout.tsx` seam |
